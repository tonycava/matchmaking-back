...
const getDirectsChats = async (
	...
	if (!useridtofetch) return res.send(new ALMResult('Missing userIdToFetch', 400));
	if (useridtofetch === id)
		return res.send(new ALMResult('Unable to retrieve your own message', 400));
...
export const getConversations = async (
	...
	return res.send(new ALMResult('OK', 200, conversations));
};
